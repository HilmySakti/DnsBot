...
exports.modStick = (media, denz, den, from) => {
	out = getRandom('.webp')
	try {
		console.log(media)
		spawn('webpmux', ['-set','exif', './src/data.exif', media, '-o', out])
		.on('exit', () => {
			denz.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: den })
			fs.unlinkSync(out)
			fs.unlinkSync(media)
		})
	} catch (e) {
		console.log(e)
		denz.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: den })
		fs.unlinkSync(media)
	}
}

exports.modMedia = (media, denz, den, from, fps) => {
	out = getRandom('.webp')
	try {
		ff(media)
		.on('error', (e) => {
			console.log(e)
			denz.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: den })
			fs.unlinkSync(media)
		})
		.on('end', () => {
			_out = getRandom('.webp')
			spawn('webpmux', ['-set','exif','./src/data.exif', out, '-o', _out])
			.on('exit', () => {
				denz.sendMessage(from, fs.readFileSync(_out), 'stickerMessage', { quoted: den })
				fs.unlinkSync(out)
				fs.unlinkSync(_out)
				fs.unlinkSync(media)
			})
